...
1. 首先用open()打开foo文件，O\_RDONLY表明只允许读，O\_LARGEFILE指定使用64-bit offset。调用成功返回文件描述符，值为3。这个值是固定的，因为每个进程已经有了3个打开的文件.（0：标准输入，1：标准输出，2：标准错误）
...
![image-20211227195649059](../../.gitbook/assets/image-20211227195649059.png)
